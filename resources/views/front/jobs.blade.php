                            ...
                            @if ($jobTypes->isNotEmpty())
                                @foreach ($jobTypes as $jobType)
                                    <div class="form-check mb-2"> 
                                        <input {{ (in_array($jobType->id,$jobTypeArray)) ? 'checked' : ''}} class="form-check-input " name="job_type" type="checkbox" value="{{ $jobType->id }}" id="job-type-id-{{ $jobType->id }}">    
...
@section('customJs')
    <script>
        $("#searchForm").submit(function(e){
            ...
            // Test if Keyword has Value
            if (keyword != ""){
                url += '&keyword='+keyword;
            }

            // Test if Location has Value
            if (location != ""){
                url += '&location='+location;
            }

            // Test if Category has Value
            if (category != ""){
                url += '&category='+category;
            }

            // Test if Experience has Value
            if (experience != ""){
                url += '&experience='+experience;
            }

            // Test if Job Types have Value
